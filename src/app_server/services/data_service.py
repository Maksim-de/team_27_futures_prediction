...
def download_prices(ticker_id: int, date_from: Optional[date], date_to: Optional[date]):
...
def get_price_table(ticker_id: str, date_from, date_to):
    data = [
...
def get_news_sentiment_table(ticker_id, date_from, date_to):
    data = ""
    return PriceTableResponse(data=data)
